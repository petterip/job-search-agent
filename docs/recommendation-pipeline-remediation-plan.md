...
| P2-1 bounded evaluation concurrency | partial | `run_llm_evaluations_concurrent` runs candidates on a bounded thread pool (one database connection per worker, `LLM_EVAL_CONCURRENCY`, default 1 = unchanged sequential path). A shared thread-safe paid budget is claimed before every provider call, a provider outage stops dispatch, and publication is request-hash keyed and conditional, so parallelism cannot bypass the cap or duplicate a publication. PostgreSQL tests measure simultaneous calls <= configured and calls <= budget; the measured throughput gain still requires authorized paired paid runs. |
