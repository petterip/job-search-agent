...
| P0-3 review backlog | implemented, execution budget-gated | Dry-run inventory is publication-independent and freezes expected request identities. `run_review_backfill` executes a bounded recovery only with `--allow-paid-backfill` plus a positive `--max-paid-calls`, blocks when the provider or profile privacy forbids hosted calls, reuses the sequential/concurrent paths, claims the paid budget before every attempt including a parse retry, and re-inventories with the cohort hash before/after. Re-running resumes because completed evaluations are stored under their request hash. Production execution (2026-09-16): a 200-call authorized backfill evaluated 129 and deferred the rest, moving the stale backlog 18,457 -> 18,328 in ~6 minutes at concurrency 4; provider 429 throttling is now classified as a deferral that stops dispatch without a long cooldown. Tests: PG publication tests, `tests/test_llm.py`. |
...
| P2-4 failure classification | implemented | `evaluate_with_parse_retry` adds one bounded parse/schema retry that claims the shared paid budget before every attempt (so a retry cannot exceed the cap), attempt counts persist, and quota/transport are not retried. `Settings.llm_config_problems()` validates provider/key/model/concurrency/retry/timeout at startup without secrets: `/health` exposes `config_warnings` and the worker exits with one actionable message. `python -m app.llm_check` (`make llm-check`) performs one billable smoke call to verify credentials, model access and structured output, and is deliberately not part of `/health`. Tests: `tests/test_llm.py`, `tests/test_health.py`, `tests/test_llm_check.py`. |
...
| P2-6 profile-calibrated prompt | mechanism verified end to end, awaits private rules | `llm_guidance.profile_rules` is validated (list of 1–400 char strings) and appended to the evaluation prompt as profile-specific calibration; request identity changes automatically when it is used, and an absent field keeps the current prompt. A real paid paired run (`app/eval_benchmark --mode prompt-variant --variant profile-rules`) confirmed identity change, decision parity 2/2 and a +92 input-token cost for the rule block. The production profile currently has no `profile_rules`, so there is nothing to enable until the owner writes calibration rules. Tests: `tests/test_matching.py`, `tests/test_profile.py`, `tests/test_eval_benchmark.py`. |
| P2-7 requirement-aware excerpt | measured, enabled | `requirement_aware_excerpt` keeps the description head plus decisive requirement/deadline sentences within the 4000-char budget; `LLM_REQUIREMENT_AWARE_EXCERPT` selects it. Measured on 6 real long listings: baseline `description[:4000]` kept only 2 of 87 decisive requirement sentences while the variant kept 63 of 87, at -484 input tokens (-1.0%) with paired decision parity 6/6 on identical inputs. Enabled in production after the measurement; the change invalidates existing evaluation identities once, which the bounded per-run budget refreshes. Tests: `tests/test_llm.py`, `tests/test_eval_benchmark.py`. |
| P2-1 bounded evaluation concurrency | measured, enabled | `run_llm_evaluations_concurrent` runs candidates on a bounded thread pool (one database connection per worker, `LLM_EVAL_CONCURRENCY`). A shared thread-safe paid budget is claimed before every provider call, a provider outage stops dispatch, and publication is request-hash keyed and conditional, so parallelism cannot bypass the cap or duplicate a publication. Measured paired paid run on the same 8-job cohort (`app/eval_benchmark --mode throughput`, OpenAI nano): sequential 18.67 s / 8 calls vs concurrency 4 at 5.05 s / 8 calls, a 3.7x speed-up with identical input tokens on both arms. Enabled at 4 in production. Tests: `tests/test_matching.py`, `tests/test_eval_benchmark.py`. |
| P2-22 route/schema docs | implemented | `tech-stack-plan.md` distinguishes shipped from proposed endpoints, `llm-hosted.md` marks escalation and batching as proposed, `feedback-learning-plan.md` marks `POST /feedback` and `PUT /profile` as not shipped, and the journal's route inventory matches the API. |
| P2-23 labelled evaluation tooling | tooling implemented, labels needed | `app/labelled_evaluation.py` consumes a private labels file and reports per-stage recall/precision by stratum with denominators and Wilson intervals; it refuses to run without labels and never commits them. `--build-sample` writes an unlabelled private template covering every required stratum as a deterministic seeded sample. Scope strata reuse the API's travel-policy-aware scope predicate and the publication predicate; jobs without a row are classified with the pipeline's own deterministic and hard gates, so only gates-passing missing jobs count as `not_retrieved` recall misses. The writer refuses repository destinations and symlinks, never overwrites without `--force`, and forces owner-only permissions. `app/labelled_review.py` can fill a template with a **simulated** LLM reviewer when no human labels exist, recording that provenance and caveat so a report can never be mistaken for human-labelled evidence. The job seeker's own labels remain authoritative. Tests: `tests/test_labelled_evaluation.py`, `tests/test_labelled_review.py`. |
