...
| P1-9a external work outside transactions | partial | Profile and job embeddings, transit routing and the LLM call now commit any open write transaction before the remote call, per-batch embedding progress is durable, and a failed candidate rolls back instead of committing an aborted transaction; instrumented tests observe no provider/HTTP call inside an open transaction. The full read-snapshot/short-write restructure (single snapshot connection, separate write phase) is not done. Tests: `tests/test_embeddings.py`, `tests/test_transit_distance.py`. |
| P2-1, P2-6, P2-7, P2-22 remainder, P2-23 | not implemented | Remaining backlog; see the task sections for dependencies. |
