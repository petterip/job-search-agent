...
| P1-7c deadline/closure lifecycle | partial | Source deadlines (`validThrough`, `applicationEndDate`/`endDate`/`deadline`/…) are extracted by the adapters into `NormalizedListing.expires_at` and stored monotonically in `jobs.expires_at`, where the freshness policy consumes them; deadlines never set catalogue status by themselves. Occurrence-level closure evidence exists in `source_scan_members.state`. Deriving canonical status across disagreeing occurrences and reopening remain. Tests: `tests/test_adapters.py`. |
| P1-9a, P1-10, P2-1, P2-6, P2-7, P2-22 remainder, P2-23 | not implemented | Remaining backlog; see the task sections for dependencies. |
